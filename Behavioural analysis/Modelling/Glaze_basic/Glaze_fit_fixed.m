...
function [pm_fit,err] = Glaze_fit_fixed(exp,subj)

% Set global variables to pass to PSO algorithm
global LLRin choices nsamps

% Path stuff
addpath(genpath([exp.modPath '/Simulations/particle_swarm_Glaze']))
loadpath = exp.dataPath;
savepath = [exp.modPath, '/Fits/']

% Define parameter estimation settings
% parameter bounds
range_p.H = [0 0.5];   % H = hazard rate
...
seeds.H = [0.1 0.03];  % good seed distributions for a selection of particles - [mean sd] %Changed from 0.08 to 0.1 to match the volatility in our task 
seeds.B = [1 0.1];
seeds.noise = [1 0.5];
