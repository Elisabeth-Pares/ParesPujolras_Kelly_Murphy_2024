...
% given combination of B (gain) and noise parameters, and
...
function [pm_fit,err] = Leak_fit_fixed(exp,subj)

% Set global variables to pass to PSO algorithm
global LLRin choices nsamps

% Path stuff
addpath(genpath([exp.modPath '/Simulations/Leak (EPP)']))
loadpath = exp.dataPath;
savepath = [exp.modPath, '/Fits/fitted_leak/']
if ~exist(savepath, 'dir'), mkdir(savepath); end

% Define parameter estimation settings 
range_p.leak = [0 0.8]%leak
range_p.B = [0.01 4.5]; %B  = gain
range_p.noise = [0.0001 10]; % Noise

mv = [0.005;
      0.01;...
      0.2]';% maximum particle velocities
...
subji = ['P' allsubj{subj}];%find(strcmp(allsubj,subj));
fprintf('Pulling behavioural data for subject %d of %d: %s\n',subji,length(allsubj),subj)
LLRin=[]; choices=[]; nsamps = [];

sdirs = dir([loadpath,subji]); sdirs = sdirs(3:end);
for s = 1:length(sdirs)
    
    sesspath = [loadpath,subji,filesep,sdirs(s).name,filesep];
...
PSOseedValue(1:n_seeded,end+1) = seeds.leak(1)+(randn(n_seeded,1).*seeds.leak(2));
if ~isempty(find(PSOseedValue(:,end)<range_p.leak(1))), PSOseedValue(find(PSOseedValue(:,end)<range_p.leak(1)),end) = range_p.leak(1); end
if ~isempty(find(PSOseedValue(:,end)>range_p.leak(2))), PSOseedValue(find(PSOseedValue(:,end)>range_p.leak(2)),end) = range_p.leak(2); end
...
par_range = [range_p.leak; range_p.B; range_p.noise];
...
% Running PSO routine
[output,te,tr] = pso_Trelea_vectorized_Glaze('Leak_cross_entropy_fitting',length(mv),mv,par_range,0,P,'goplotpso',PSOseedValue);
...
save([savepath,subji,'_fixed_fit.mat'],'pm_fit','err','gen','P','PSOseedValue','seeds','range_p','te','tr')
