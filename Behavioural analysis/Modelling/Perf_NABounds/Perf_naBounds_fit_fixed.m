...
function [pm_fit,err] = Perf_naBounds_fit_fixed(exp,subj)

% Set global variables to pass to PSO algorithm
global LLRin choices nsamps

% Path stuff
addpath(genpath([exp.modPath '/Simulations/Perf_NABounds (EPP)']))
loadpath = exp.dataPath;
savepath = [exp.modPath, '/Fits/fitted_perf_naBounds/']
if ~exist(savepath, 'dir'), mkdir(savepath); end

% Define parameter estimation settings -  set equal to Glaze model (EPP)
range_p.A = [0.01,50] %A = threshold
range_p.B = [0.01 4.5]; %B  = gain
range_p.noise = [0.0001,50];%[0.0001 10]; % Noise
...
PSOseedValue(1:n_seeded,end+1) = seeds.A(1)+(randn(n_seeded,1).*seeds.A(2));
if ~isempty(find(PSOseedValue(:,end)<range_p.A(1))), PSOseedValue(find(PSOseedValue(:,end)<range_p.A(1)),end) = range_p.A(1); end
if ~isempty(find(PSOseedValue(:,end)>range_p.A(2))), PSOseedValue(find(PSOseedValue(:,end)>range_p.A(2)),end) = range_p.A(2); end
...
par_range = [range_p.A; range_p.B; range_p.noise];
...
% Running PSO routine
[output,te,tr] = pso_Trelea_vectorized_Glaze('Perf_naBounds_cross_entropy_fitting',length(mv),mv,par_range,0,P,'goplotpso',PSOseedValue);
