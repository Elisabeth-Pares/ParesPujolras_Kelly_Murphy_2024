...
function [pm_fit,err] = Perf_fit_fixed(exp,subj)

% Set global variables to pass to PSO algorithm
global LLRin choices nsamps

% Path stuff
addpath(genpath([exp.modPath '/Simulations/Perf_basic']))
loadpath = exp.dataPath;
savepath = [exp.modPath, '/Fits/fitted_perf/']
...
par_range = [range_p.B; range_p.noise];
...
% Running PSO routine
[output,te,tr] = pso_Trelea_vectorized_Glaze('Perf_cross_entropy_fitting',length(mv),mv,par_range,0,P,'goplotpso',PSOseedValue);
