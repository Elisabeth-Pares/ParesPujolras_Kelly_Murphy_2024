...
function [LPRout,surprise,prior] = accPerf_fast(LLRin,startpoint,Stype,pIn,H)
...
if strcmp(Stype,'pCP')  % analytically-derived change-point probability
    pR = l2p([zeros(size(LPRout,1),1) LPRout],'p'); %prob right
    pL = l2p([zeros(size(LPRout,1),1) LPRout],'n'); %prob left
