...
for nsubj = 1:20
 [gaDat] = VT_Consistency_ERP(nsubj,exp,par, gaDat) 
end
save([exp.dataPath, 'VT_Consistency_' par.dataType 'gaData.mat'], 'gaDat');

%% Data for permutation test 
load([exp.dataPath, 'VT_Consistency_' par.dataType 'gaData.mat']);

clear permData;
for s = 1:20
    permData.cp{s} = squeeze(gaDat.cons.data(s,:)-nanmean(gaDat.cons.data(s,1:10),2));
    permData.nocp{s}= squeeze(gaDat.incons.data(s,:)-nanmean(gaDat.incons.data(s,1:10),2));
end 
...
ylabel(['|\Delta\Psi|']); xticks([1,2]); xticklabels({'Cons', 'Incons'});
...
for s = 1:20
    ...
    for field = 1:length(ff);
        thisField = ff{field}
        baselines =  nanmean(gaDat.(thisField).data(s,1:10),2);
