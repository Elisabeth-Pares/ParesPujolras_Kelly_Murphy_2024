...
cfg.minnbchan   = [0];
cfg.latency     = par.clusterTime; %Analysis period
...
cluster.prior = stat1; 
cluster.llr = stat2; 
cluster.llrxsurprise = stat3;

%% Plot topography 

if strcmp(par.averageChan, 'no')
    allStats = {'stat1', 'stat2', 'stat3'};
    allEffects = {'all_trl_prior', 'all_trl_LLR', 'all_trl_LLRxSurprise'};
    effectNames = {'prior', '|LLR|', '|LLR*|Surprise'};
    f = figure
    for stats = 1:length(allStats)
